...
public abstract class WorkTask <Params, Progress, Result> {
    ...
    public WorkTask() {
        ...
        this.mWorker = new WorkTask.WorkerRunnable(null) {
            public Result call() throws Exception {
                TLog.v("WorkTask", (!TextUtils.isEmpty(WorkTask.this.taskId)?WorkTask.this.taskId:"") + " call()");
                WorkTask.this.mTaskInvoked.set(true);
                Process.setThreadPriority(10);
                return WorkTask.this.postResult(WorkTask.this.doInBackground(this.mParams));
            }
        };
        this.mFuture = new FutureTask(this.mWorker) {
            protected void done() {
                try {
                    TLog.v("WorkTask", (!TextUtils.isEmpty(WorkTask.this.taskId)?WorkTask.this.taskId:"") + " done()");
                    Object t = this.get();
                    TLog.v("WorkTask", (!TextUtils.isEmpty(WorkTask.this.taskId)?WorkTask.this.taskId:"") + " done(), result = " + t);
                    WorkTask.this.postResultIfNotInvoked(t);
                } catch (InterruptedException var2) {
                    Log.w("WorkTask", var2);
                } catch (ExecutionException var3) {
                    throw new RuntimeException("An error occured while executing doInBackground()", var3.getCause());
                } catch (CancellationException var4) {
                    WorkTask.this.postResultIfNotInvoked((Object)null);
    ...
    private void postResultIfNotInvoked(Result result) {
        ...
        TLog.v("WorkTask", (!TextUtils.isEmpty(this.taskId)?this.taskId:"") + " postResultIfNotInvoked(), result = " + result + ", wasTaskInvoked = " + wasTaskInvoked);
        if(!wasTaskInvoked) {
    ...
    protected void onFinished() {
        TLog.d("WorkTask", String.format("%s --->onFinished()", new Object[]{TextUtils.isEmpty(this.taskId)?"run ":this.taskId + " run "}));
    }

    public abstract Result workInBackground(Params... var1) throws TaskException;

    private Result doInBackground(Params... params) {
        TLog.d("WorkTask", String.format("%s --->doInBackground()", new Object[]{TextUtils.isEmpty(this.taskId)?"run ":this.taskId + " run "}));
    ...
    protected final void onPreExecute() {
        TLog.d("WorkTask", String.format("%s --->onPrepare()", new Object[]{TextUtils.isEmpty(this.taskId)?"run ":this.taskId + " run "}));
        this.onPrepare();
    }

    protected final void onPostExecute(Result result) {
        if(this.exception == null) {
            TLog.d("WorkTask", String.format("%s --->onSuccess()", new Object[]{TextUtils.isEmpty(this.taskId)?"run ":this.taskId + " run "}));
            this.onSuccess(result);
        } else if(this.exception != null) {
            TLog.d("WorkTask", String.format("%s --->onFailure(), \nError msg --->", new Object[]{TextUtils.isEmpty(this.taskId)?"run ":this.taskId + " run ", this.exception.getMessage()}));
    ...
    protected void onCancelled() {
        TLog.d("WorkTask", String.format("%s --->onCancelled()", new Object[]{TextUtils.isEmpty(this.taskId)?"run ":this.taskId + " run "}));
    }
    ...
    static {
        ...
        SERIAL_EXECUTOR = new WorkTask.SerialExecutor(null);
    ...
    private static class WorkThread extends Thread {
        ...
        WorkThread(Runnable target, String name) {
            super(target, name);
            ++threadCount;
            TLog.v("WorkTask", "WorkThread count = " + threadCount);
        }
